...
class JankenIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        class janken():
            def main(cls,my_cmd,e,nemy_cmd):
                logger.debug("自分の手　" + str(my_cmd))
        
                logger.debug("相手の手　" + str(enemy_cmd))
                #ジャンケン処理
                if enecotd == e   enemy_cmd == 1 and my_cmd == 2 or 
                     s  plinnereturn(winner)
               else :戦う
                   winny_勝負,
                        "デュエル",
                class Dmaバトル)
            def __in]inner,myfish_status,enemy_status):
